...
def restore_file(client, path, cutoff_datetime, is_deleted, verbose=False):
    try:
        ...
        print('no revisions on: ', path.encode('utf8'))
        return
...
def restore_folder(client, path, cutoff_datetime, verbose=False, do_recurse=True):
    if verbose:
    ...
    for item in folder.entries:
        if isinstance(item, dropbox.files.FolderMetadata):
            if do_recurse:
                ...
                print('not recursing into ', item.path_lower)
        else:
            # TODO: Restore functionality whereby a file that didn't exist at that
            # point in time will get deleted.
...
def main():
    ...
    restore_folder(client, root_path, cutoff_datetime, verbose=True, do_recurse=False) # TEMP: disable recursion
